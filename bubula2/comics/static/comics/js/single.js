	...
	$('#comic-date').
		...
		one('click', function(e) {
			...
			var elem = $(this),
				...
				appendTo(content);
			Dajaxice.comics.get_comic_list(function(data) {
				loading.remove();
				var list = $('<ul/>').appendTo(content);
				$.each(data.comics, function() {
					var item = $('<li/>').appendTo(list),
						itemA = $('<a/>').
							attr('href', global.comicsUrl+this.id.toString()+'/').
							appendTo(item);
					if (global.comicId == this.id) {
...
//						appendTo(item);
	...
	$(document).ready(function($) {
		...
		$.getScript('http://www.ohnorobot.com/js/2033.js', function() {
			...
			transcribe('apqYa6imImgok2033', global.comicUrl, global.comicTitle);
			document.write = orgW;
		});
	});
})(jQuery);