...
TPL="$1"
MAIN_DATA_FILE="$2"
DATA_DIR=$(dirname "$MAIN_DATA_FILE")
OUTPUT=${3:-'/dev/stdout'}
    ...
    cat "$DATA_FILE" | while read LINE; do
        if echo "$LINE" | grep -q '^%'; then
            ...
            'include')
                ...
                process_data_file "$DATA_DIR/$INC_FILE"
                if [ $? -ne 0 ]; then
...
DATA=$(process_data_file "$MAIN_DATA_FILE")

cat "$TPL" | sed "$(echo "$DATA" | sed -r 's/[\/&]/\\\0/g; s/([A-Z_]+)=(.*)/s\/%\1%\/\2\/g/')" > "$OUTPUT"
